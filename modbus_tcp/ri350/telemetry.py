...
def decode_SW1(value: int) -> str:
    """ПЧ слово состояния 1 (адрес: 2100H)."""
    states = {
        0x0001: "Вперед",
        0x0002: "Назад",
        0x0003: "Останов",
    ...
    return states.get(value, f"Неизвестно (0x{value:04X})")


def decode_SW2(value: int) -> Dict[str, Any]:
    """ПЧ слово состояния 2 (адрес: 2101H)."""
    motor_sel_map = {0: "Двигатель 1", 1: "Двигатель 2"}
    ctrl_src_map = {0: "Клавиатура", 1: "Терминал", 2: "Связь"}
    vector_map = {
        0: "Вектор 0",
        1: "Вектор 1",
    ...
    return {
        ...
        "mode": (
            ...
            if (value & (1 << 8))
            else "Контроль скорости"
        ),
        "position": bool(value & (1 << 9)),
        "vector": vector_map.get(
            (value >> 10) & 0b11, f"Код {(value >> 10) & 0b11}"
        ),
    }
