"""
опрос индикатора т42 по модбас рту
"""
import serial
import struct
...
def request():
    ser = serial.Serial('COM18', 115200, timeout=1, bytesize=serial.EIGHTBITS, stopbits=1)

    # стандартный запрос регистров ввода в 16-ричном формате modbus rtu (01 = адрес, 04 - регистры ввода
    # 0000 - стартовый адрес, 0012 - 18 значений, контрольная сумма crc16 modbus переворачивается
    # https://crccalc.com/ - расчет crc)

    # запрос всех (18) значений
    request = bytes.fromhex('01 04 0000 0012 7007')
